...
species<-"ROMU"
plotdata<-dplyr::filter(herbs.AvgQuadFreq, `USDA Plants Code` %in% species) 
...
ggplot(plotdata , aes(x= year, y = MeanPropPerZone, colour= Zone)) + geom_point() +geom_line()+
  ...
  facet_wrap(~Island)+
  theme_classic()+
  labs(title = paste(plotdata$`Common Name`[plotdata$`USDA Plants Code` %in% species]), y= "Mean proportion of quadrats +- SE", x="Year")

kable(plotdata, caption = "Plotted herb quad fre. data")
...
species<-"CEOR7"
plotdata<-dplyr::filter(herbs.AvgCover, `USDA Plants Code` %in% species) 
...
ggplot(plotdata , aes(x= year, y = MeanCoverPerZone, colour= Zone)) + geom_point() +geom_line()+
  ...
  facet_wrap(~Island)+
  theme_classic()+
  labs(title = paste(plotdata$`Common Name`[plotdata$`USDA Plants Code` %in% species]), y= "Mean cover (%) +- SE", x="Year")
