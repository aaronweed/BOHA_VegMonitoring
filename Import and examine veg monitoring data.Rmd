...
herbs<-bind_rows(Thomp15$`HERB LAYER` %>%  dplyr::select(-starts_with("Note"), -First_point_cm), Thomp16$`HERB LAYER` %>%  dplyr::select(-starts_with("Note")),Thomp18$`HERB LAYER` %>%  dplyr::select(-starts_with("Note")),Thomp19$`HERB LAYER` %>%  dplyr::select(-starts_with("Note")), 
...
herb.QuadFreqTrans<-herbsSpecies %>% group_by(Island, Zone, year, Transect, `USDA Plants Code`) %>% tally(name = "Quads_Present") %>% ungroup() %>% 
...
group_by( `USDA Plants Code`,Island, Zone, year) %>%  
  summarise(MeanPropPerZone = mean(PropQuadsPerTrans, na.rm = TRUE), SDPropZone = sd(PropQuadsPerTrans, na.rm = TRUE), Transects = n()) %>% 
...
species<-"CEOR7"
data<-dplyr::filter(herb.QuadFreqTrans, `USDA Plants Code` %in% species) 

ggplot(data , aes(x= year, y = MeanPropPerZone, colour= Zone)) + geom_point() +geom_line()+
  #scale_color_viridis_d()
  facet_wrap(~Island)+
  theme_classic()+
  labs(title = paste(data$`Common Name`[data$`USDA Plants Code` %in% species]), y= "Mean proportion of quadrats", x="Year")
